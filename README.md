...
Vercel runs the public site from the bundled fleet seed without creating SQLite files in its read-only application directory. Node.js 22 is pinned in `package.json`; fleet data and locale dictionaries are explicitly included in the server-function bundle. Set `NEXT_PUBLIC_SITE_URL` to `https://dubailuxurycarrentals.ae` (not the local-preview URL). If omitted, the server uses this production domain on Vercel.

Catalogue browsing, event pages, monthly pricing and WhatsApp enquiries work without a writable local database. Online reservation forms use Neon Postgres on Vercel and SQLite locally; email is not required. Follow [RESERVATIONS-DEPLOYMENT.md](RESERVATIONS-DEPLOYMENT.md) to connect storage and configure private document encryption and administrator access. The forms remain visible while an unconfigured submission API returns 503 instead of reporting false success. Partner applications and hosted fleet editing are separate and remain unavailable on Vercel. Do not point `ZAVI_DATA_DIR` at `/tmp`: temporary storage would lose customer records.
